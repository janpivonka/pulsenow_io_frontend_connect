...
const Assets = () => {
  ...
  return (
    <div className="space-y-10 p-4 md:p-12 max-w-7xl mx-auto text-slate-900">
      ...
      {/* TABS FILTER */}
      <div className="flex bg-slate-100 p-1.5 rounded-[1.5rem] w-fit overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAssets.map(asset => (
          <div
            ...
            className="group bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:scale-[1.02] transition-all cursor-pointer relative overflow-hidden"
          >
            <div className="flex justify-between items-start relative z-10">
              <div className="min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-2xl font-black tracking-tighter uppercase italic group-hover:text-blue-600 transition-colors">
                    {asset.symbol}
                  </h3>
              ...
              <div className="text-right">
                <p className="text-2xl font-mono font-black text-slate-900 leading-none mb-2">
                  ${asset.currentPrice?.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </p>
                <div className={`text-[11px] font-black flex items-center justify-end gap-1 px-2 py-0.5 rounded-lg ${asset.changePercent >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
            ...
            <div className="absolute -right-4 -bottom-4 text-slate-50 font-black text-6xl select-none opacity-40 group-hover:text-blue-50 transition-colors italic">
