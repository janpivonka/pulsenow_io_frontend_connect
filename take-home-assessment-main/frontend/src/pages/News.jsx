...
const News = () => {
  ...
  return (
    <div className="space-y-10 p-4 md:p-12 max-w-7xl mx-auto text-slate-900">
      ...
      {/* NOVÝ ZJEDNODUŠENÝ HEADER */}
      <PageHeader badge="Market Intelligence" title="News Stream">
        <SearchInput onSearch={setSearchTerm} placeholder="Search news..." />
      </PageHeader>

      {/* CATEGORY FILTER */}
      <div className="flex bg-slate-100 p-1.5 rounded-[1.5rem] overflow-x-auto no-scrollbar gap-1 w-fit">
        {categories.map((cat) => (
          <button
            key={cat}
        ...
        {filteredNews.length > 0 && activeCategory === 'All' && !searchTerm && (
          ...
          >
            ...
            <div className="relative z-10 space-y-4 max-w-3xl">
              ...
              <h2 className="text-3xl md:text-5xl font-black text-white leading-tight tracking-tighter group-hover:text-blue-400 transition-colors">{filteredNews[0].title}</h2>
              <p className="text-slate-400 text-lg md:text-xl font-medium line-clamp-2 italic">{filteredNews[0].summary}</p>
        ...
        <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredNews.slice((activeCategory === 'All' && !searchTerm) ? 1 : 0).map((item) => (
            ...
            >
              <div className="space-y-4">
                <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest px-3 py-1 bg-blue-50 rounded-lg w-fit block">{item.category}</span>
                <h3 className="text-xl font-black leading-tight tracking-tight">{item.title}</h3>
                <p className="text-slate-500 text-sm line-clamp-3 font-medium leading-relaxed">{item.summary}</p>
              </div>
          ...
          {filteredNews.length === 0 && (
            <div className="col-span-full py-20 text-center bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
              <p className="text-slate-400 font-black uppercase tracking-widest text-sm">No news found for "{searchTerm}"</p>
            </div>
          )}
        </div>
      </div>
