...
const Portfolio = () => {
  ...
  const stats = useMemo(() => {
    if (!portfolio.assets) return { myPositions: [], total: 0, pl: 0, plPercent: 0, cash: 0 };
    ...
    const positions = portfolio.assets.map(item => {
      ...
      return {
        ...
        plPercent: ((currentPrice - item.avgBuyPrice) / item.avgBuyPrice) * 100
      };
  ...
  if (!portfolio.assets) return <div className="p-20 text-center font-black animate-pulse uppercase tracking-widest text-xs">Syncing Terminal...</div>;
  ...
  return (
    <div className="space-y-10 p-4 md:p-12 max-w-7xl mx-auto text-slate-900">
      ...
      <PageHeader badge="Institutional Account" title="Portfolio">
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm min-w-[300px]">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1 text-right">Live Net Worth</p>
          <div className="text-right">
            <div className="text-4xl md:text-5xl font-black tracking-tighter italic leading-none mb-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:border-blue-100 transition-colors">
            <p className="text-[9px] font-black uppercase text-slate-400 tracking-[0.2em] mb-1">{s.label}</p>
            <p className="text-xl font-black italic uppercase leading-none">{s.val}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden transition-all">
        <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-xs">Live Engine</span>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {stats.myPositions.map((pos) => (
                <tr key={pos.symbol} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="font-black text-lg uppercase group-hover:text-blue-600 transition-colors italic leading-none">{pos.symbol}</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-1">{pos.name}</div>
                  </td>
                  <td className="px-8 py-6 text-right font-mono font-bold text-slate-500">
                    {pos.amount.toLocaleString()}
                  </td>
                  <td className="px-8 py-6 text-right">
                    <div className="font-black text-lg leading-none">${pos.currentValue.toLocaleString()}</div>
                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter mt-1">Avg: ${pos.avgPrice.toLocaleString()}</div>
                  </td>
                  <td className="px-8 py-6 text-right">
                    ...
                    <div className={`text-[10px] font-black uppercase px-2 py-0.5 rounded mt-1 inline-block ${pos.profitLoss >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {aiCoreInsights.slice(0, 3).map(insight => (
            <div key={insight.id} className="bg-slate-900 rounded-[2.5rem] p-8 text-white border border-slate-800 hover:border-blue-900 transition-all shadow-xl group">
