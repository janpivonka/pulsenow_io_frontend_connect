...
const Modal = ({ item: initialItem, type, onClose }) => {
  ...
  // Dynamically find the current item in the real-time stream
  const liveItem = [...stocks, ...crypto].find(s => s.symbol === initialItem?.symbol) || initialItem;
  ...
  // Format alert timestamp in English
  const formatAlertDate = (dateString) => {
    ...
    // If alert happened within the last hour, show relative time
    if (diffInMinutes < 60 && diffInMinutes >= 0) {
      return `${diffInMinutes}m ago`;
    }

    // Otherwise show absolute time in English format
                   ...
                   {liveItem.alerts?.length > 0 ? (
                     <div className="space-y-4">
                       {liveItem.alerts.map((alert) => (
                         <div key={alert.id} className={`p-5 rounded-2xl border-l-[6px] shadow-sm bg-white border-white ${
                           alert.impact === 'positive' ? 'border-l-emerald-500' :
                           alert.impact === 'negative' ? 'border-l-rose-500' : 'border-l-amber-500'
                         }`}>
                           <div className="flex justify-between items-center mb-2">
                             <span className={`text-[10px] font-black uppercase tracking-tighter ${
                               alert.impact === 'positive' ? 'text-emerald-600' :
                               alert.impact === 'negative' ? 'text-rose-600' : 'text-amber-600'
                     ...
                     <div className="text-center py-10">
                        <p className="text-slate-400 font-medium italic">No active alerts for this asset.</p>
                     </div>
                   )}
                </div>
              </div>
